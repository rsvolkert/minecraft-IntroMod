package com.rvolkert.intromod.entity.custom;
...
public class ChomperEntity extends Monster implements GeoEntity {
    ...
    private PlayState attackPredicate(AnimationState animationState) {
        if(this.swinging && animationState.getController().getAnimationState().equals(AnimationController.State.STOPPED)) {
            ...
            animationState.getController().setAnimation(RawAnimation.begin().then("animation.chomper.attack", Animation.LoopType.PLAY_ONCE));
            this.swinging = false;
        }
